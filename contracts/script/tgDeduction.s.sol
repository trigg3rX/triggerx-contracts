...
contract TGDeduction is Script {
    ...
    function deployTriggerGasRegistry() public {
        ...
        bytes32 implementation_salt = keccak256(
            abi.encodePacked("triggerGasRegistrysdsrs")
        );

        bytes memory implementation_code = type(TriggerGasRegistry)
            .creationCode;
        ...
        console2.log(
            "TriggerGasRegistry implementation deployed to:",
            implementation
        );
        console2.log(
            "TriggerGasRegistry proxy deployed to:",
    ...
    function deployJobRegistry() public {
        ...
        bytes32 implementation_salt = keccak256(
            abi.encodePacked("jobRegistrysfsdf")
        );
    ...
    function purchaseTG() public {
        ...
        console2.log(
            "ETH deposited: ",
            vm.toString(finalETHBalance - initialETHBalance)
        );
    ...
    function executeFunction(uint256 jobId) public {
        ...
        uint256 initialETHBalance = triggerGasRegistry.getBalance(jobOwner);
        console2.log(
            "Initial ETH balance before execution: ",
            vm.toString(initialETHBalance)
        );
        ...
        uint256 finalETHBalance = triggerGasRegistry.getBalance(jobOwner);
        console2.log(
            "Final ETH balance after execution: ",
            vm.toString(finalETHBalance)
        );
        console2.log(
            "ETH deducted: ",
            vm.toString(initialETHBalance - finalETHBalance)
        );

        // Verify the deduction
        require(
            finalETHBalance == initialETHBalance - ethAmountToDeduct,
            "ETH should be deducted correctly"
        );
