...
contract TaskExecutionHubSecurityTest is Test {
    ...
    function test_Security_OnlyKeeperCanExecuteFunction() public {
        ...
        MockJobRegistry mockJobRegistry = MockJobRegistry(
            address(taskExecutionHub.jobRegistry())
        );
        MockTriggerGasRegistry mockTriggerGasRegistry = MockTriggerGasRegistry(
            address(taskExecutionHub.triggerGasRegistry())
        );
    ...
    function test_Security_CannotAddSelfAsKeeper() public {
        ...
        bytes memory maliciousPayload = abi.encode(
            TaskExecutionHub.ActionType.REGISTER,
            attacker
        );
    ...
    function test_Security_MessageFromInvalidSource() public {
        ...
        vm.expectRevert(
            abi.encodeWithSignature("NoPeer(uint32)", uint32(99999))
        );
        vm.prank(address(mockEndpoint));
    ...
    function test_Security_FunctionExecutionWithMaliciousTarget() public {
        uint256 jobId = 1;
        uint256 tgAmount = 100;
        MaliciousTarget maliciousTarget = new MaliciousTarget(
            payable(address(taskExecutionHub))
        );
    ...
    function test_Security_CannotExecuteOnTaskExecutionHubItself() public {
        ...
        bytes memory data = abi.encodeWithSelector(
            TaskExecutionHub.withdraw.selector,
            attacker,
            1 ether
        );
        ...
        // This should fail gracefully (emit event) because the call to itself will fail
        // (withdraw is protected by onlyOwner, and executeFunction calls it as the hub contract, which is owner,
        // BUT executeFunction is nonReentrant and withdraw is nonReentrant, so it might fail due to reentrancy guard if it was reentrant)
        // Actually, withdraw is nonReentrant. executeFunction is nonReentrant.
        // When executeFunction calls target.call, it's an external call.
        // If target is address(this), it calls withdraw.
        // withdraw requires onlyOwner. address(this) IS the owner (it owns itself? No, owner is set in initialize).
        // The owner is `owner` variable (address(0x1)). address(this) is NOT the owner.
    ...
    function test_Security_GasGriefingProtection() public {
        ...
        bytes memory data = abi.encodeWithSelector(
            GasGriefingContract.gasGriefing.selector
        );
        ...
        // But it should NOT revert the whole transaction, just the execution
        // Note: In a real scenario, if it runs out of gas, the whole tx reverts.
        // But here we are simulating an internal revert in the called contract.
        // If the called contract reverts (even due to out of gas in the sub-call), the low-level call returns false.
    ...
    function test_Security_BroadcastWithInsufficientFunds() public {
        ...
        vm.prank(owner);
        taskExecutionHub.withdraw(
            payable(address(0x9999)),
            address(taskExecutionHub).balance
        ); // Use proper address, not precompile
    ...
    function test_Security_WithdrawReentrancyProtection() public {
        DrainAttacker drainAttacker = new DrainAttacker(
            payable(address(taskExecutionHub))
        );
    ...
    function test_Security_CannotCorruptKeeperState() public {
        ...
        vm.expectRevert(
            abi.encodeWithSignature("OnlyEndpoint(address)", attacker)
        );
        vm.prank(attacker);
    ...
    function test_Security_FullKeeperLifecycleIntegrity() public {
        ...
        bytes memory unregisterPayload = abi.encode(
            TaskExecutionHub.ActionType.UNREGISTER,
            testKeeper
        );
        vm.prank(address(mockEndpoint));
        taskExecutionHub.lzReceive(
